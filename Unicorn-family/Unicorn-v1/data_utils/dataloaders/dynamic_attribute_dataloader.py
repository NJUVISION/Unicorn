...
from data_utils.attribute.inout import read_h5, read_ply_ascii, read_ply_o3d, read_h5_dynamic
from geometry_dataloader import InfSampler
from data_utils.attribute.color_format import rgb2yuv, yuv2rgb, rgb2YCoCg, YCoCg2rgb
...
def collate_dynamic_bframe_pointcloud_fn(list_data):
    new_list_data = []
    num_removed = 0
    for data in list_data:
        if data is not None:
    ...
    if len(list_data) == 0:
        raise ValueError('No data in the batch')
    coords0, feats0, coords1, feats1, coords2, feats2 = list(zip(*list_data))
    coords_batch0, feats_batch0 = ME.utils.sparse_collate(coords0, feats0)
    coords_batch1, feats_batch1 = ME.utils.sparse_collate(coords1, feats1)
    coords_batch2, feats_batch2 = ME.utils.sparse_collate(coords2, feats2)

    return coords_batch0, feats_batch0, coords_batch1, feats_batch1, coords_batch2, feats_batch2
...
class DynamicBFramePCDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx in self.cache:
            ...
            coords0, feats0, coords1, feats1 = read_h5_dynamic(filedir)
            # cache
            self.cache[idx] = (coords0, feats0, coords1, feats1)
...
class BFramePCDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx in self.cache:
            coords0, feats0, coords1, feats1, coords2, feats2 = self.cache[idx]
        else:
            coords0, feats0, coords1, feats1, coords2, feats2 = read_h5_dynamic(filedir)
            # cache
            self.cache[idx] = (coords0, feats0, coords1, feats1, coords2, feats2)
            # cache_percent = int((len(self.cache) / len(self)) * 100)
        coords0 = coords0.astype("int32")
        feats0 = feats0.astype("float32")
        coords1 = coords1.astype("int32")
        feats1 = feats1.astype("float32")
        coords2 = coords2.astype("int32")
        ...
        if feats0.shape[-1] == 3:
            ...
            if self.color_format == 'rgb':
                feats0 = feats0 / 255. * out_range
                feats1 = feats1 / 255. * out_range
                feats2 = feats2 / 255. * out_range
            elif self.color_format == 'yuv':
                feats0 = rgb2yuv(feats0, out_range=out_range)
                feats1 = rgb2yuv(feats1, out_range=out_range)
                feats2 = rgb2yuv(feats2, out_range=out_range)
            elif self.color_format == 'y':
                feats0 = rgb2yuv(feats0, out_range=out_range)[:, 0:1]
                feats1 = rgb2yuv(feats1, out_range=out_range)[:, 0:1]
                feats2 = rgb2yuv(feats2, out_range=out_range)[:, 0:1]
            elif self.color_format == 'ycocg':
                feats0 = rgb2YCoCg(feats0)
                feats1 = rgb2YCoCg(feats1)
                feats2 = rgb2YCoCg(feats2)
            elif self.color_format == 'Y':
                feats0 = rgb2YCoCg(feats0)[:, 0:1]
                feats1 = rgb2YCoCg(feats1)[:, 0:1]
                feats2 = rgb2YCoCg(feats2)[:, 0:1]
        if feats0.shape[-1] == 1:
            if self.normalize:
                feats0 = feats0 / 255.
                feats1 = feats1 / 255.
        ...
        return (coords0, feats0, coords1, feats1, coords2, feats2)
...
def make_bframe_data_loader(dataset, batch_size=1, shuffle=True, num_workers=1, repeat=False,
                            collate_fn=collate_dynamic_bframe_pointcloud_fn):
    args = {
        'batch_size': batch_size,
        'num_workers': num_workers,
        'collate_fn': collate_fn,
