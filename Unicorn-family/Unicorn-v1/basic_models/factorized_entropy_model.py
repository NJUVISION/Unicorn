...
class EntropyBottleneck(nn.Module):
    ...
    def _quantize(self, inputs, mode="straight", step=1.0):
    ...
    def forward(self, inputs, quantize_mode="straight", quantize_step=1.0):
...
def get_entropy(data, Qstep=1):
    # data = data/Qstep
    bits_sum = 0
    for idx in range(data.shape[-1]):
        # normalization: positive integers
        data_raw = data[:,idx]
        curr_data = data_raw.reshape(-1).astype('int')
        keys = np.sort(np.unique(curr_data))
        curr_data_norm = curr_data.copy()
        for i, k in enumerate(keys):
            curr_data_norm[curr_data==k] = i
        # statistic
        statistic = Counter(curr_data_norm)
        freq_table = {}
        ...
        probs = pmf[curr_data_norm]
        bits = -np.log2(np.array(probs))
        bits = bits.reshape(data_raw.shape)
        bits_sum+=bits.sum()
