...
import numpy as np
from data_utils.sparse_tensor import isin, create_new_sparse_tensor, sort_sparse_tensor
...
class UpSampler(torch.nn.Module):
    def __init__(self, in_channels=1, channels=32, out_channels=32, kernel_size=3, 
                ...
                deconv_kernel_size=2, stride=2, expand_coordinates=True, block_type='conv', knn=16):
        super().__init__()
        if block_type=='conv':
...
class UpSampler1Stage(torch.nn.Module):
    def __init__(self, in_channels=1, channels=32, kernel_size=3, block_layers=3, stage=1, block_type='conv'):
        super().__init__()
        self.upsampler = UpSampler(
                ...
                deconv_kernel_size=[2,2,2], stride=[2,2,2], block_type=block_type)
    ...
    def istopk_local(self, data, k=1):
        ...
        mask[indices] = True
        # print('DBG!!!istopk_local\n'*100)
        
        return mask.bool().to(data.device)

    def istopk_global(self, data, k):
        """input data is probability
    ...
    def upsample(self, x_low, num_points):
        ...
        # print('DBG!!!upsample\n'*10, prob.shape)
        if cfg.only_global_topk:
            mask = self.istopk_global(prob, k=num_points)
        else:
            mask = self.istopk_local(prob, k=1)
...
class UpSampler8Stage(torch.nn.Module):
    def __init__(self, in_channels=1, channels=32, kernel_size=5, block_layers=3, stage=8, block_type='conv'):
        super().__init__()
        self.stage = stage
        ...
        self.block_list = torch.nn.ModuleList([
            self.make_block(in_channels=channels, channels=channels, out_channels=channels, 
                            kernel_size=kernel_size, block_layers=block_layers, block_type=block_type) for i in range(self.stage)])
        self.classifier = Classifier(channels)
        self.pruning = ME.MinkowskiPruning()

        self.unpooling = ME.MinkowskiPoolingTranspose(kernel_size=2, stride=2, expand_coordinates=True, dimension=3)
        self.unpooling_skip = ME.MinkowskiPoolingTranspose(kernel_size=2, stride=2, expand_coordinates=False, dimension=3)
        self.sumpooling = ME.MinkowskiSumPooling(kernel_size=2, stride=2, dimension=3)
    ...
    def make_block(self, in_channels=32, channels=32, out_channels=32, kernel_size=3, block_layers=3, knn=16, block_type='conv'):
        ...
        #             kernel_size=kernel_size, stride=1, bias=True, dimension=3),
        #         ME.MinkowskiReLU(inplace=True),
        #         PointTransformerBlock(block_layers=block_layers, 
        #                         in_channels=channels, channels=channels, out_channels=channels,
        #                         knn=knn),
        #         ME.MinkowskiConvolution(in_channels=channels, out_channels=out_channels,
        #             kernel_size=kernel_size, stride=1, bias=True, dimension=3))
        ...
        #             kernel_size=kernel_size, stride=1, bias=True, dimension=3),
        #         ME.MinkowskiReLU(inplace=True),
        #         GraphNet(channels=channels, knn=knn),
        #         ME.MinkowskiConvolution(in_channels=channels, out_channels=out_channels,
        #             kernel_size=kernel_size, stride=1, bias=True, dimension=3))
        ...
        #             kernel_size=kernel_size, stride=1, bias=True, dimension=3),
        #         ME.MinkowskiReLU(inplace=True),
        #         DGCNN(in_channels=channels, channels=channels, out_channels=channels, knn=knn),
        #         ME.MinkowskiConvolution(in_channels=channels, out_channels=out_channels,
        #             kernel_size=kernel_size, stride=1, bias=True, dimension=3))
