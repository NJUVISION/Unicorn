...
class Trainer():
    def __init__(self, model, prefix='tp', init_ckpt='', rootdir=None, pretrained_modules=None, 
                frozen_modules=None, lr=0.0001, check_time=10, device='cuda', DBG=False, 
                clip_grad=False, clip_value=10, weight_decay=0, save_time=10, cfg=None):
        ...
        if cfg is None:
            ...
            self.init_ckpt = cfg.init_ckpt
            self.pretrained_modules = cfg.pretrained_modules
            self.frozen_modules = cfg.frozen_modules
            self.lr = cfg.lr
            self.check_time = cfg.check_time# minute
            self.DBG = bool(cfg.DBG)
            self.clip_grad = bool(cfg.clip_grad)
            self.clip_value = cfg.clip_value
            self.weight_decay = cfg.weight_decay
            self.save_time = cfg.save_time

        self.device = device
        self.model = model.to(device)
        if rootdir is None: rootdir = os.path.split(os.path.split(__file__)[0])[0]
        # self.ckptdir = os.path.join(rootdir, 'ckpts', self.prefix)
        self.ckptdir = os.path.join(rootdir, self.prefix)
        ...
        _ = self.load_state_dict(self.init_ckpt)# load ckpt
        self.epoch = 0
        self.record_set = {}
    ...
    def save_model(self):
        ...
        torch.save({'model': self.model.state_dict()}, 
                    os.path.join(self.ckptdir, str(int(np.ceil(self.epoch/self.save_time)*self.save_time))+'.pth'))
    ...
    def set_optimizer(self):
        ...
        optimizer = torch.optim.Adam(params_lr_list, weight_decay=self.weight_decay)
        if self.DBG:
            print('='*30, 'weight_decay', '='*30, self.weight_decay)
    ...
    def record(self, main_tag, global_step):
        ...
        for k, v in self.record_set.items():
            # print('DBG!!!record', k, np.array(v).shape)
            self.record_set[k]=np.mean(np.array(v), axis=0).squeeze()
        for k, v in self.record_set.items():
            self.logger.info(k+': '+str(np.round(v, 6)))
            if len(v.shape) > 1: self.logger.info(k+' sum: '+str(np.round(v.sum(axis=-1), 6).tolist()))
        # tensorboard            
        tag_scalar_dict = {}
    ...
    def clip_gradient(self, max_norm=None, clip_value=None, DBG=True):
        if DBG: max_grad = max(p.grad.data.abs().max() for p in self.model.parameters() if p.grad is not None)
        # clip by norm
        ...
        if DBG:
            max_grad2 = max(p.grad.data.abs().max() for p in self.model.parameters() if p.grad is not None)
    ...
    def train(self, dataloader):
        ...
        for batch_step, data in enumerate(tqdm(dataloader)):
            ...
            try:
                ...
                if loss is None:
                    self.logger.info('loss is None'*100 + '!'*1000)
                    continue
                
                loss.backward()# backward

                if self.clip_grad:
                    self.clip_gradient(clip_value=self.clip_value, DBG=self.DBG)# 1

                self.optimizer.step()# optimize
            except (MemoryError, ValueError, RuntimeError, IndexError) as e:
            # except (MemoryError) as e:
                torch.cuda.empty_cache()
                torch.cuda.empty_cache()
                time.sleep(2)
                self.logger.info('!'*1000+' except: '+str(e)+'!'*1000)
                continue
            # check & record
    ...
    def test(self, dataloader, main_tag='Test'):
        ...
        for _, data in enumerate(tqdm(dataloader)):
            ...
            except (RuntimeError, ValueError, MemoryError, IndexError) as e:
            # except (MemoryError) as e:
                torch.cuda.empty_cache()
