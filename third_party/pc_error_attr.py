...
def pc_error(infile1, infile2, res, normal=False, color=True, lidar=False, show=False):
    ...
    command = rootdir_tmc13 + '/pc_error_d' + ' -a '+infile1+ ' -b '+infile2 + \
...
def avs_pcc_pc_evalue(infile1, infile2, res, show=False):
    ...
    start = time.time()
    subp=subprocess.Popen(rootdir_tmc13+'/avs-pcc-pc_evalue '+ 
                          ' -f1 '+infile1+ 
                          ' -f2 '+infile2+ 
                          ' -pk '+str(res-1), 
                          shell=True, stdout=subprocess.PIPE)
